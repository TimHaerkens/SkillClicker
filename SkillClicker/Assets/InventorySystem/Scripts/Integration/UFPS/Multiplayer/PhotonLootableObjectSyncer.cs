...
namespace Devdog.InventorySystem.Integration.UFPS
{
    ...
    public partial class PhotonLootableObjectSyncer : Photon.MonoBehaviour
    {
        ...
        public virtual void Awake()
        {
            ...
            triggerer.OnTriggerUsed += (player) => { photonView.RPC("OnTriggerUsed", PhotonTargets.Others); };
            triggerer.OnTriggerUnUsed += (player) => { photonView.RPC("OnTriggerUnUsed", PhotonTargets.Others); };
        }

        [PunRPC]
        private void OnTriggerUsed()
        {
            triggerer.DoVisuals();
        }

        [PunRPC]
        private void OnTriggerUnUsed()
        {
        ...
        private void SetLootableItemsClients(string itemIDsString, PhotonMessageInfo info)
        {
            ...
            for (int i = 0; i < itemIDs.Length; i++)
            {
                var item = UnityEngine.Object.Instantiate<InventoryItemBase>(ItemManager.database.items[int.Parse(itemIDs[i])]);
        ...
        private void LootableObjectItemRemoved(int slot)
        {
            ...
            lootable.lootUI.SetItems(lootable.items, true);
        }
    }
}
