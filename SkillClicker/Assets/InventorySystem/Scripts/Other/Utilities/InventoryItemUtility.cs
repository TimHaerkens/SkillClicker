...
namespace Devdog.InventorySystem
{
    public class InventoryItemUtility
    {
        ...
        public static InventoryItemAmountRow[] EnforceMaxStackSize(IEnumerable<InventoryItemAmountRow> itemRows)
        {
            var list = new List<InventoryItemAmountRow>();
            foreach (var row in itemRows)
            {
                uint stackCount = row.amount;
                while (stackCount > 0)
                {
                    var row2 = new InventoryItemAmountRow(row.item, stackCount);
