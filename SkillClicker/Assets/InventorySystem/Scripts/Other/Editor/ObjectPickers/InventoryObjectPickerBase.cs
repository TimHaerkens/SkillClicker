...
namespace Devdog.InventorySystem.Editors
{
    public abstract class InventoryObjectPickerBase<T> : EditorWindow where T : class
    {
        ...
        private List<Action<T>> _onPickObjectList = new List<Action<T>>(2);

        private event Action<T> _OnPickObject;
        public event Action<T> OnPickObject
        {
            add
            {
                _OnPickObject += value;
                _onPickObjectList.Add(value);
            }
            remove
            {
                _OnPickObject -= value;
                _onPickObjectList.Remove(value);
            }
        }
        ...
        {
            // Remove prev. events
            while (_onPickObjectList.Count > 0)
            {
                _OnPickObject -= _onPickObjectList[0];
                _onPickObjectList.RemoveAt(0);
            }
