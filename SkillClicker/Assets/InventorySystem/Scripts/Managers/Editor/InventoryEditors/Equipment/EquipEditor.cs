...
namespace Devdog.InventorySystem.Editors
{
    public class EquipEditor : IInventorySystemEditorCrud
    {
        ...
        public EquipEditor(string name, EditorWindow window)
        {
            ...
            typeList = new UnityEditorInternal.ReorderableList(ItemManager.database != null ? ItemManager.database.equipStatTypes : new string[] { }, typeof(System.Type), false, true, true, true);
            ...
            typeList.drawElementCallback += (rect, index, active, focused) =>
            {
                ...
                var statTypes = ItemManager.database.equipStatTypes;
            ...
            {
                var l = new List<string>(ItemManager.database.equipStatTypes);
                l.Add(null);
                ItemManager.database.equipStatTypes = l.ToArray();
                list.list = ItemManager.database.equipStatTypes;
            ...
            resultList = new UnityEditorInternal.ReorderableList(ItemManager.database != null ? ItemManager.database.equipStats : new InventoryEquipStat[] { }, typeof(InventoryEquipStat), true, true, false, false);
            resultList.drawHeaderCallback += rect =>
            {
                ...
                r2.x += r2.width;
                EditorGUI.LabelField(r2, "UI Color");

                r2.x += r2.width;
                EditorGUI.LabelField(r2, "Icon");
            };
            ...
            resultList.drawElementCallback += (rect, index, active, focused) =>
            {
                ...
                var stat = ItemManager.database.equipStats[index];
                ...
                r2.x += r2.width + 5;
                stat.color = EditorGUI.ColorField(r2, stat.color);

                r2.x += r2.width + 5;
                stat.sprite = InventoryEditorUtility.SimpleObjectPicker<Sprite>(r2, "", stat.sprite, false, false);
            };
        }
        ...
        public virtual void Focus()
        {
            ...
            typeList.list = ItemManager.database.equipStatTypes;
            resultList.list = ItemManager.database.equipStats;
        }
        ...
        public virtual void Draw()
        {
            ...
            {
                var oldList = new List<InventoryEquipStat>(ItemManager.database.equipStats);
                var displayList = new List<InventoryEquipStat>(64);
                foreach (var type in ItemManager.database.equipStatTypes)
                {
                    ...
                    foreach (var field in fields)
                    {
                        ...
                        if (attr.Length > 0)
                        {
                            ...
                            if (old.Count == 0)
                            {
                                displayList.Add(new InventoryEquipStat() { name = field.Name, typeName = type, fieldInfoName = field.Name, fieldInfoNameVisual = field.ReflectedType.Name + "." + field.Name, show = false, category = "Default" });
                            }
                            else
                            {
                ...
                ItemManager.database.equipStats = displayList.ToArray();
                resultList.list = ItemManager.database.equipStats; // Update list view
            }
