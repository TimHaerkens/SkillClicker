            ...
            if (_amount < 0f)
            {
                ...
                foreach (var lookup in parentContainer.lookups)
                {
                    if (conversionDict.ContainsKey(lookup._currencyID))
                    {
                        ...
                        float amountInOtherCurrency = lookup.amount*conversionDict[lookup._currencyID];
                        if (amountInOtherCurrency > Mathf.Abs(_amount))
                        {
                            float currentAmount = _amount;
                            currentAmount *= conversionDict[lookup._currencyID];
                            currentAmount = Mathf.Abs(currentAmount);

                            _amount = 0f;
                            lookup.amount -= currentAmount;
                            return;
                        }
                    }
                }
