...
{
    [AddComponentMenu("InventorySystem/Layout/Dynamic Layout Group")]
    public class DynamicLayoutGroup : LayoutGroup
    {
        ...
        private InventoryUIItemWrapperBase[] _wrappers = new InventoryUIItemWrapperBase[0];
...
//            SetLayoutVertical(true);
        ...
        {
            var l = new List<InventoryUIItemWrapperBase>(rectChildren.Count);
            foreach (var child in rectChildren)
            {
                var c = child.GetComponent<InventoryUIItemWrapperBase>();
                Assert.IsNotNull(c, "Object without wrapper component in collection container. This isn't allowed. Make sure all objects inside the collection container have a wrapper component.");
                l.Add(c);
            }
        ...
        private void SetCellsAlongAxis(int axis, bool forceRebuild = false)
        {
            ...
            if (_wrappers.Length != rectChildren.Count)
            {
                CacheWrappers();
            }
            else
            {
                ...
                {
                    var c = rectChildren[i].GetComponent<InventoryUIItemWrapperBase>();
                    if (c != null)
                    {
                        if (_wrappers[i].item != c.item)
                        {
                            shouldRebuild = true;
                            break;
                        }
...
//                cellCountX = Mathf.Max(1, Mathf.FloorToInt((width - padding.horizontal + spacing.x + 0.001f) / (cellSize.x + spacing.x)));
            ...
            for (int i = 0; i < rectChildren.Count; i++)
            {
                ...
                uint layoutCols = 1;
                uint layoutRows = 1;
                if (_wrappers[i].item != null)
                {
                    layoutCols = _wrappers[i].item.layoutSizeCols;
                    layoutRows = _wrappers[i].item.layoutSizeRows;
                }
